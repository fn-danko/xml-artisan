...
`XML`, `Sel`, `Node` con operazioni dirette, serializzazione, XPath contestuale, resilienza completa.

### v1.1 — Data binding

`BoundSel<T>`, `JoinedSel<T>`, `JoinConfig<T>` con pattern join, matching posizionale e per key function.
...
XML Artisan fa parte di una famiglia di librerie XML:

- **XML Spelunker** — Processa enormi XML gerarchici in modalità streaming, estraendo porzioni di interesse senza caricare l'intero documento in memoria.
- **XML Artisan** — Manipolazione fluent di documenti XML in-memory con selezioni e data binding.

La visione futura prevede l'integrazione tra le due: Spelunker estrae frammenti da file di grandi dimensioni, Artisan li lavora con la sua API fluent.
